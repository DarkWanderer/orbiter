...
INT WINAPI WinMainImpl(HINSTANCE hInstance, HINSTANCE _, PSTR strCmdLine, INT nCmdShow)
{
#ifdef INLINEGRAPHICS
...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE _, PSTR strCmdLine, INT nCmdShow)
{
	__try {
		WinMainImpl(hInstance, _, strCmdLine, nCmdShow);
	}
...
void Orbiter::KbdInputImmediate_OnRunning(char* kstate)
{
	...
	if (keymap.IsLogicalKey(kstate, OAPI_LKEY_KillMainRetro)) {
		g_focusobj->SetThrusterGroupLevel(THGROUP_MAIN, 0.0);
