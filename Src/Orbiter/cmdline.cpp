...
void CommandLine::ParseCmdLine(const PSTR cmdLine)
{
	...
	auto args = CommandLineToArgvA(cmdLine, &nArgs);
	...
	for (int i = 0; i < nArgs; i++) {
		...
		if (len < 2 || arg[0] != '-')
			continue;
		if (arg[1] == '-') // "Long" parameter "--scenario"
		{
			auto eqPos = strstr(arg + 2, "=");
			if (eqPos) // --scenario=MyScenario
			{
				*eqPos = '\0'; // replace '=' with null byte
				option.strKey = arg + 2;
				option.strVal = eqPos + 1;
			}
			else // --scenario MyScenario
			{
				option.strKey = arg;
				i++;
				if (i < nArgs) {
					option.strVal = args[i];
				}
				else {
					option.strVal = "";
				}
